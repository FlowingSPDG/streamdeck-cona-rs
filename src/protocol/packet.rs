...
impl PacketWriter {
    ...
    pub fn pad_to_packet_size(mut data: Vec<u8>) -> Vec<u8> {
...
impl PacketReader {
    ...
    pub fn read_u16_le(packet: &[u8], offset: usize) -> Result<u16> {
        if offset + 2 > packet.len() {
            return Err(Error::Protocol(format!(
                "Cannot read u16 at offset {}: packet too short",
                offset
            )));
        }
    ...
    pub fn read_slice(packet: &[u8], offset: usize, length: usize) -> Result<&[u8]> {
        if offset + length > packet.len() {
            return Err(Error::Protocol(format!(
                "Cannot read {} bytes from offset {}: packet too short",
                length, offset
            )));
        }
