...
async fn handle_message(
    ...
    if let Some(report_id) = parse_get_report_request(&message.payload, message.flags) {
        ...
        match report_id {
            ...
            0x08 => {
                // Studio is a PRIMARY PORT device, so it should NOT respond to 0x08 (secondary port)
                // The client uses Promise.race to send both 0x80 and 0x08 concurrently.
                // If Studio responds to 0x08, the client may incorrectly set isPrimary=false,
                // which causes it to query 0x1c (Device 2 info) and potentially misidentify the device.
                // 
                // FIX: Studio should only respond to 0x80 (primary port), not 0x08 (secondary port)
                log(LogLevel::Warn, "[Server] GET_REPORT 0x08 received, but Studio is a primary port device. Ignoring (should only respond to 0x80).");
                // Don't send response - let the client receive 0x80 response instead
                return Ok(());
            }
            0x1a => {
